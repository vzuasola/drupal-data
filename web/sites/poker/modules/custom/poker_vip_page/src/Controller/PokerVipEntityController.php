...
class PokerVipEntityController extends ControllerBase implements ContainerInjectionInterface {
  ...
  public function revisionPageTitle($poker_vip_entity_revision) {
    $poker_vip_entity = $this->entityManager()->getStorage('poker_vip_entity')->loadRevision($poker_vip_entity_revision);
    return $this->t('Revision of %title from %date', ['%title' => $poker_vip_entity->label(), '%date' => format_date($poker_vip_entity->getRevisionCreationTime())]);
  }
  ...
  public function revisionOverview(PokerVipEntityInterface $poker_vip_entity) {
    ...
    $build['#title'] = $has_translations ? $this->t('@langname revisions for %title', ['@langname' => $langname, '%title' => $poker_vip_entity->label()]) : $this->t('Revisions for %title', ['%title' => $poker_vip_entity->label()]);
    $header = [$this->t('Revision'), $this->t('Operations')];

    $revert_permission = (($account->hasPermission("revert all poker vip entity revisions") || $account->hasPermission('administer poker vip entity entities')));
    $delete_permission = (($account->hasPermission("delete all poker vip entity revisions") || $account->hasPermission('administer poker vip entity entities')));
    ...
    foreach (array_reverse($vids) as $vid) {
      ...
      if ($revision->hasTranslation($langcode) && $revision->getTranslation($langcode)->isRevisionTranslationAffected()) {
        ...
        if ($vid != $poker_vip_entity->getRevisionId()) {
          $link = $this->l($date, new Url('entity.poker_vip_entity.revision', ['poker_vip_entity' => $poker_vip_entity->id(), 'poker_vip_entity_revision' => $vid]));
        }
        else {
        ...
        if ($latest_revision) {
          ...
          if ($revert_permission) {
            $links['revert'] = [
              ...
              Url::fromRoute('entity.poker_vip_entity.translation_revert', ['poker_vip_entity' => $poker_vip_entity->id(), 'poker_vip_entity_revision' => $vid, 'langcode' => $langcode]) :
              Url::fromRoute('entity.poker_vip_entity.revision_revert', ['poker_vip_entity' => $poker_vip_entity->id(), 'poker_vip_entity_revision' => $vid]),
            ];
          }
          ...
          if ($delete_permission) {
            $links['delete'] = [
              'title' => $this->t('Delete'),
              'url' => Url::fromRoute('entity.poker_vip_entity.revision_delete', ['poker_vip_entity' => $poker_vip_entity->id(), 'poker_vip_entity_revision' => $vid]),
            ];
          }
