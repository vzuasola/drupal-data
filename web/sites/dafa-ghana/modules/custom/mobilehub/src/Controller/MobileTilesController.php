...
class MobileTilesController extends ControllerBase implements ContainerInjectionInterface {
  ...
  public function revisionPageTitle($mobile_tiles_revision) {
    $mobile_tiles = $this->entityManager()->getStorage('mobile_tiles')->loadRevision($mobile_tiles_revision);
    return $this->t('Revision of %title from %date', ['%title' => $mobile_tiles->label(), '%date' => format_date($mobile_tiles->getRevisionCreationTime())]);
  }
  ...
  public function revisionOverview(MobileTilesInterface $mobile_tiles) {
    ...
    $build['#title'] = $has_translations ? $this->t('@langname revisions for %title', ['@langname' => $langname, '%title' => $mobile_tiles->label()]) : $this->t('Revisions for %title', ['%title' => $mobile_tiles->label()]);
    $header = [$this->t('Revision'), $this->t('Operations')];

    $revert_permission = (($account->hasPermission("revert all mobile tiles revisions") || $account->hasPermission('administer mobile tiles entities')));
    $delete_permission = (($account->hasPermission("delete all mobile tiles revisions") || $account->hasPermission('administer mobile tiles entities')));
    ...
    foreach (array_reverse($vids) as $vid) {
      ...
      if ($revision->hasTranslation($langcode) && $revision->getTranslation($langcode)->isRevisionTranslationAffected()) {
        ...
        if ($vid != $mobile_tiles->getRevisionId()) {
          $link = $this->l($date, new Url('entity.mobile_tiles.revision', ['mobile_tiles' => $mobile_tiles->id(), 'mobile_tiles_revision' => $vid]));
        }
        else {
        ...
        if ($latest_revision) {
          ...
          if ($revert_permission) {
            $links['revert'] = [
              ...
              Url::fromRoute('entity.mobile_tiles.translation_revert', ['mobile_tiles' => $mobile_tiles->id(), 'mobile_tiles_revision' => $vid, 'langcode' => $langcode]) :
              Url::fromRoute('entity.mobile_tiles.revision_revert', ['mobile_tiles' => $mobile_tiles->id(), 'mobile_tiles_revision' => $vid]),
            ];
          }
          ...
          if ($delete_permission) {
            $links['delete'] = [
              'title' => $this->t('Delete'),
              'url' => Url::fromRoute('entity.mobile_tiles.revision_delete', ['mobile_tiles' => $mobile_tiles->id(), 'mobile_tiles_revision' => $vid]),
            ];
          }
