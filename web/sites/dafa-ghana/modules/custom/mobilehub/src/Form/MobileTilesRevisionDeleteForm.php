...
class MobileTilesRevisionDeleteForm extends ConfirmFormBase {
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    $this->logger('content')->notice('Mobile tiles: deleted %title revision %revision.', [
      '%title' => $this->revision->label(),
      '%revision' => $this->revision->getRevisionId()]);
    drupal_set_message(t('Revision from %revision-date of Mobile tiles %title has been deleted.', [
      '%revision-date' => format_date($this->revision->getRevisionCreationTime()),
      '%title' => $this->revision->label()]));
    ...
    if ($this->connection->query('SELECT COUNT(DISTINCT vid) FROM {mobile_tiles_field_revision}
