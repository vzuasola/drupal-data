...
class AuthorizationProfileForm extends EntityForm {
  ...
  public function form(array $form, FormStateInterface $form_state) {
    ...
    /* @var AuthorizationProfile $authorization_profile */
   ...
   */
  public function buildEntityForm(array &$form, FormStateInterface $form_state, AuthorizationProfile $authorization_profile) {
    $form['label'] = array(
      '#type' => 'textfield',
      '#title' => $this->t('Label'),
      '#maxlength' => 255,
    ...
    if ($provider_options) {
      ...
      drupal_set_message($this->t('There are no Provider plugins available for the Authorization. You will need to install and enable something like the LDAP Authorization Provider module that ships with LDAP.'), 'error');
      $form = array();
    }
    ...
    if ($consumer_options) {
      ...
      drupal_set_message($this->t('There are no authorization Consumer plugins
      available. You can enable the Authorization Drupal Roles submodule to provide integration with core user roles or write your own using that as a template.'), 'error');
      $form = array();
    }
  }
   ...
   *   The profile that is being created or edited.
   */
  public function buildProviderConfigForm(array &$form, FormStateInterface $form_state, AuthorizationProfile $authorization_profile) {
   ...
   *
   * @param AuthorizationProfile $authorization_profile
   *   The profile that is being created or edited.
   */
  public function buildConsumerConfigForm(array &$form, FormStateInterface $form_state, AuthorizationProfile $authorization_profile) {
  ...
  public function buildConditionsForm(array &$form, FormStateInterface $form_state, AuthorizationProfile $authorization_profile) {
    ...
    if (property_exists($this,'syncOnLogon') && $this->syncOnLogon)  {
   ...
   * @param AuthorizationProfile $authorization_profile
   */
  public function buildMappingForm(array &$form, FormStateInterface $form_state, AuthorizationProfile $authorization_profile) {
    if (
