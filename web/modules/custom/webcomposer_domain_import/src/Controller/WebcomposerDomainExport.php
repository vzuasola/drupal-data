...
class WebcomposerDomainExport extends ControllerBase {
  ...
  public function domain_export_get_parsed_data() {
      $result = array();
      $language = array();
      $service  = \Drupal::service('webcomposer_domain_import.export');
      $groups = $service->get_domain_groups();
      $domains = $service->get_domain_groups_with_domains();
      //kint($domains);die;
      // $placeholders = $service->_matterhorn_domain_get_all_variables();
      ...
      $result['languages'] = $service->excel_get_languages($language);

      // kint($result['languages']);die;
      //$result['groups'] = $service->excel_get_domain_groups($groups);
      // kint($result['groups']);die;
      $result['domains'] = $service->excel_get_domains($domains);
